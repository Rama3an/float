...
fs.readFile(arg[2], (err, input) => {
    ...
    for (let i = 0; i < input.length; ++i) {
        if (input.length != 3) {
            ...
            result = finishFloat(calc(input));
            break;
        }

    }

    fs.writeFile(arg[3], result, (err) => {
        if (err) {
            console.err(err);
            return;
        }
...
function conv(input) {
    ...
    if (input > ((2 - Math.pow(2, -23)) * Math.pow(2, 127)) || input == Number.POSITIVE_INFINITY) {
        sign = '0';
        mantissa = '00000000000000000000000';
        degree = '11111111';
    } else if (input < -((2 - Math.pow(2, -23)) * Math.pow(2, 127)) || input == Number.NEGATIVE_INFINITY) {
        sign = '1';
        degree = '11111111';
        mantissa = '00000000000000000000000';
...
function Mantissa(input) {
    ...
    if (input.length > 1 && index[0] == "0" && index[0].length == 1){
        let indexOfOne = index[1].indexOf("1") + 1;
        (mantissa = indexOfOne > 126)
            ? input.substring(128, input.length)
            : mantissa = index[1].substring(indexOfOne);
    }
    else {
